...
var VideoSteps = {
    ...
    startVideo: async function (opts) {
        ...
        this.video = this.video || new VideoRecorder();
    ...
    removeVideo: function (opts) {
        ...
        if (!this.video.filePath || !fs.existsSync(this.video.filePath)) {
        ...
        fs.unlinkSync(this.video.filePath);

        if (check) {
            expect(fs.existsSync(this.video.filePath),
